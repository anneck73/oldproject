...
abstract class MangopayBaseTest extends TestCase
{
    /**
    ...
    {
        if (null === self::$JohnsBankingAliasIBAN) {
            $john = $this->getJohn();
    ...
    {
        if (null === self::$JohnsPayInCardWeb) {
            $wallet = $this->getJohnsWallet();
            $user = $this->getJohn();
    ...
    {
        if (null === self::$JohnsWalletWithMoney) {
            $john = $this->getJohn();
    ...
    protected function getPaylineCorrectRegistartionData($cardRegistration)
    {
        ...
        if (false === $response && 0 !== curl_errno($curlHandle)) {
    ...
    {
        if (null === self::$JohnsAccount || false === self::$JohnsAccount->Active) {
            $john = $this->getJohn();
    ...
    {
        if (null === self::$JohnsPayOutBankWire) {
            $wallet = $this->getJohnsWallet();
            $user = $this->getJohn();
            $account = $this->getJohnsAccount();
    ...
    {
        if (null === self::$JohnsPayOutForCardDirect) {
    ...
    {
        if (null === self::$JohnsCardRegistration) {
            $user = $this->getJohn();
     ...
     * Creates card registration object.
     *
     * @param mixed|null $idempotencyKey
     *
    ...
    {
        if (null === self::$JohnsPayInPaypalWeb) {
            $wallet = $this->getJohnsWallet();
            $user = $this->getJohn();
    ...
    protected function assertIdenticalInputProps($entity1, $entity2)
    {
        if (is_a($entity1, '\MangoPay\Address')) {
            $this->assertSame($entity1->AddressLine1, $entity2->AddressLine1);
            $this->assertSame($entity1->AddressLine2, $entity2->AddressLine2);
            $this->assertSame($entity1->City, $entity2->City);
            $this->assertSame($entity1->Country, $entity2->Country);
            $this->assertSame($entity1->PostalCode, $entity2->PostalCode);
            $this->assertSame($entity1->Region, $entity2->Region);
    ...
    {
        if (null === self::$PayInPaymentDetailsCard) {
    ...
    {
        if (null === self::$PayInExecutionDetailsWeb) {
