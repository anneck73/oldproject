...
use Mealmatch\ServiceTasksBundle\Services\PayoutHomeMealsService;
use Psr\Log\LoggerInterface;
use Symfony\Component\Console\Command\Command;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Output\OutputInterface;

class MangopayPayoutHomeMealsCommand extends Command
{
    /**
     * @var PayoutHomeMealsService
     */
    private $payoutHomeMealsService;
    /**
    ...
    public function __construct(
        ...
        PayoutHomeMealsService $payoutHomeMealsService,
        ...
        $this->payoutHomeMealsService = $payoutHomeMealsService;
    ...
    {
        $this
            ->setName('mm:mangopay:payout_homemeals')
    ...
    {
        $output->writeln('Mealmatch Mangopay Payout HomeMeals Command (started)');
        $input->validate();
        $resultsCollection = $this->payoutHomeMealsService->findHomeMealPayouts();
        $count = 1;
        ...
        foreach ($resultsCollection as $result) {
            ...
            if ($result['isDue']) {
                ...
                $valid = $hostPaymentProfile->hasMangopayBankAccountId();
                if (!$valid) {
                    $this->logger->debug('Skip>>Host Payment Profile has no BankAccount!!!>>>'.$mealticket->getNumber());
                    $this->logger->debug('Payment Profile of Host: '.$host->getUsername().' is not valid for Payout!');
        ...
        $output->writeln('Mealmatch Mangopay Payout HomeMeals Command (finished:'.$count.')');
    }
